...
class GenericObjectType(BaseModel):
    ...
    preferred_name: Optional[TranslatableString] = Field(
...
class ConsumableType(GenericObjectType):
    ...
    consume: dict[
        Literal["xp", "bits", "gems", "hearts", "wheels", "blitz_energy", "tls"], int
    ] = Field(default_factory=dict)
    time: int = 0
    skip_cost: int = 0
    farm: Optional[list[FarmCost]] = Field(default=None, exclude_if=lambda v: v is None)
    critter: Optional[ConsumableCritter] = Field(
        default=None, exclude_if=lambda v: v is None
    )
...
class CostumeType(GenericObjectType):
    ...
    parts: dict[Literal["body", "mane", "tail"], GameObjectId | None] = Field(
...
class FortuneShop(BaseModel):
    ...
    items: dict[FortuneShopRarities, dict[GameObjectId, FortuneShopItem]] = Field(
        default_factory=dict
    )
...
class GameObjects(BaseModel):
    ...
    avatar_frame: CategoryData[AvatarFrameType] = Field(
        default_factory=CategoryData[AvatarFrameType]
    )
    background: CategoryData[BackgroundType] = Field(
        default_factory=CategoryData[BackgroundType]
    )
    background_frame: CategoryData[BackgroundFrameType] = Field(
        default_factory=CategoryData[BackgroundFrameType]
    )
    cutie_mark: CategoryData[CutieMarkType] = Field(
        default_factory=CategoryData[CutieMarkType]
    )
    ...
    booster: CategoryData[BoosterType] = Field(
        default_factory=CategoryData[BoosterType]
    )
    token: CategoryData[TokenType] = Field(default_factory=CategoryData[TokenType])
    consumable: CategoryData[ConsumableType] = Field(
        default_factory=CategoryData[ConsumableType]
    )
    costume: CategoryData[CostumeType] = Field(
        default_factory=CategoryData[CostumeType]
    )
    costume_part: CategoryData[CostumePartType] = Field(
        default_factory=CategoryData[CostumePartType]
    )
